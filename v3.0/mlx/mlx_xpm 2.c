...
#define	RETURN	{ if (colors) free(colors); if (map) free(map); \
...
char	**mlx_int_str_to_wordmap(char *str)
{
  char	**map;
  int	pos;
  int	nb_word;
  ...
  if (!(map = malloc((1+nb_word)*sizeof(*map))))
    return ((char **)0);
  nb_word = 0;
  pos = 0;
  while (pos<len)
...
void	*mlx_int_parse_xpm(void *xvar,void *info,int info_size,char *(*f)(), int *width, int *height)
{
  ...
  if (!(line = f(info,&pos,info_size)) ||
      !(map = mlx_int_str_to_wordmap(line)) || !(*width = atoi(map[0])) ||
      !(*height = atoi(map[1])) || !(nc = atoi(map[2])) ||
      !(cpp = atoi(map[3])) )
    RETURN;
  free(map);
  map = 0;
  ...
  while (i--)
    {
      if (!(line = f(info,&pos,info_size)) ||
	  !(map = mlx_int_str_to_wordmap(line+cpp)) )
	RETURN;
      j = 0;
      while (map[j] && strcmp(map[j++],"c"));

      if (!map[j])
	RETURN;

      rgb_col = mlx_int_get_text_rgb(map[j], map[j+1]);
      if (method)
