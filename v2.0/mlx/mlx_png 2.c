...
int	mipng_crc(unsigned char *ptr, int len)
{
  ...
  i = 0;
  while (i < len+4)
    crc = crc_maple[(crc ^ ptr[(i++)+4]) & 0xff] ^ (crc >> 8);
  crc ^= 0xffffffffL;
